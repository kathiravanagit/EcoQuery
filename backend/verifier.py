    ...
    def verify_completion(
        ...
        if latency_seconds <= 0 or completion_tokens <= 0:
            return {
                "status": "verified",
                "confidence": 0.95,
                "reason": "Insufficient latency data to dispute threshold",
        ...
        if observed_tps > threshold["max_tps"] * 1.6 and "mini" not in model_id and "haiku" not in model_id:
            issues.append(f"TPS ({observed_tps}) far exceeds {model_id} max ({threshold['max_tps']})")
            confidence -= 0.35

        if observed_tps < threshold["min_tps"] * 0.15 and observed_tps > 0:
            issues.append(f"TPS ({observed_tps}) suspiciously low for {model_id}")
            confidence -= 0.2

        if latency_ratio > 3.0:
            issues.append(f"Latency ({latency_seconds:.1f}s) is {latency_ratio:.1f}x expected ({threshold['avg_latency_s']:.1f}s)")
            confidence -= 0.15
        ...
        return {
            ...
            "reason": f"Throughput ({observed_tps} TPS) matches {model_id} estimated threshold.",
            "adjusted_co2_g": reported_co2_g,
            "observed_tps": observed_tps,
            "flagged": False,
            "integrity_hash": integrity_hash,

...
def _route_sync(tier: str, mode: str = "eco"):
    return asyncio.run(route_query(tier, 50, mode))


def test_route_query_simple_eco():
    result = _route_sync("simple", "eco")
...
def test_route_query_complex_performance():
    result = _route_sync("complex", "performance")
    assert result is not None
    assert "model" in result
    assert "region" in result
...
def test_route_query_medium():
    result = _route_sync("medium", "eco")
    assert result is not None
    assert "model" in result
    assert isinstance(result["model"]["tier"], str)


def test_route_query_all_modes():
    for tier in ("simple", "medium", "complex"):
        for mode in ("eco", "performance"):
            result = _route_sync(tier, mode)
            assert result is not None
            assert "model" in result
            assert "region" in result
            assert "savings" in result
...
def test_route_query_unknown_tier_fallback():
    result = _route_sync("unknown_tier_xyz", "eco")
    assert result is not None
    assert "model" in result
    assert "region" in result
...
def test_route_query_returns_mode():
    result = _route_sync("simple", "eco")
...
def test_compute_savings_baseline_constant():
    ...
    # Baseline should differ with different prompt_length
